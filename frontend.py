...
with st.sidebar:
    ...
    for tech in ["🔗 LangGraph", "🧠 Open AI · gpt-oss-20b", "🐘 My Sql", "🔍 Tavily Search", "✈️ AviationStack"]:
    ...
    for step in ["① Validation Agent","② Flight Agent", "③ Hotel Agent", "④ Itinerary Agent", "⑤ Final Agent"]:
...
if generate:
    if not user_query.strip():
            ...
            for node_name, state_update in chunk.items():
                ...
                with st.status(f"{icon}  {label}", state="complete", expanded=True):
                    if node_name == "validation_agent":
                        is_valid = state_update.get("is_valid_query")
                        if is_valid is False:
                            msgs = state_update.get("messages", [])
                            if msgs:
                                st.warning(msgs[-1].content)
                            validation_failed = True
                        else:
                            st.markdown("✅ Query validated. Proceeding to agents...")
        ...
        <div class="metric-row">
            <div class="metric-box"><div class="metric-val">{agents_run_count}</div><div class="metric-lbl">Agents Run</div></div>
            <div class="metric-box"><div class="metric-val">{collected['llm_calls']}</div><div class="metric-lbl">LLM Calls</div></div>
            <div class="metric-box"><div class="metric-val">{"❌" if validation_failed else "✅"}</div><div class="metric-lbl">Status</div></div>
        </div>
        """, unsafe_allow_html=True)
...
"""
            ...
            with info_col:
                st.markdown(f"<div class='save-bar'>📄 Saved locally to <code>{filename}</code></div>",
                            unsafe_allow_html=True)
